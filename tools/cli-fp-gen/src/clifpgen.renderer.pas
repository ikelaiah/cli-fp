...
function RenderRootCommandUnit(const Spec: TProjectSpec): string;
...
begin
  ...
  try
    ...
    if Spec.HasRootCommand then
      Lines.Add('  RootCommand: ICommand;');
    Lines.Add('begin');
    Lines.Add('  try');
    if Spec.HasRootCommand then
    begin
      Lines.Add('    RootCommand := CreateGeneratedRootCommand;');
      Lines.Add('    App := CreateCLIApplication(' +
        PascalStringLiteral(Spec.AppName) + ', ' +
        PascalStringLiteral(Spec.AppVersion) + ', RootCommand);');
    end
    else
      Lines.Add('    App := CreateCLIApplication(' +
        PascalStringLiteral(Spec.AppName) + ', ' +
...
begin
  ...
  try
    ...
    if Spec.HasRootCommand then
      UsesUnits.Add(MakeRootCommandUnitName(Spec.AppName));
    ...
    Lines.Add('{ Code generated by cli-fp-gen. DO NOT EDIT. }');
    if Spec.HasRootCommand then
      Lines.Add('function CreateGeneratedRootCommand: ICommand;');
    ...
    begin
      Lines.Add('function CreateGeneratedRootCommand: ICommand;');
      Lines.Add('var');
      Lines.Add('  RootCommand: ' +
        MakeRootCommandUnitName(Spec.AppName) + '.TRootCommand;');
      Lines.Add('begin');
      Lines.Add('  RootCommand := ' +
        MakeRootCommandUnitName(Spec.AppName) + '.TRootCommand.Create;');
      Lines.Add('  RootCommand.UpdateDescription(' +
        PascalStringLiteral(Spec.RootCommand.Description) + ');');
      for j := 0 to Spec.RootCommand.Parameters.Count - 1 do
        Lines.Add('  ' + RenderParameterCall('RootCommand',
          Spec.RootCommand.Parameters[j]));
      Lines.Add('  Result := RootCommand;');
      Lines.Add('end;');
      Lines.Add('');
    end;
...
var
  ...
  UnitName: string;
begin
  UnitName := MakeRootCommandUnitName(Spec.AppName);
  ...
  try
    ...
    Lines.Add('');
    Lines.Add('{ User-owned root command stub created by cli-fp-gen. Safe to edit. }');
    Lines.Add('type');
    Lines.Add('  TRootCommand = class(TBaseCommand)');
    Lines.Add('  public');
    ...
    Lines.Add('implementation');
    Lines.Add('');
    Lines.Add('constructor TRootCommand.Create;');
    Lines.Add('begin');
    Lines.Add('  inherited Create('''', ' +
      PascalStringLiteral(Spec.RootCommand.Description) + ');');
    Lines.Add('end;');
    Lines.Add('');
    Lines.Add('function TRootCommand.Execute: Integer;');
    Lines.Add('begin');
    Lines.Add('  ' + PascalWriteLnLiteral(
